...
echo ">>> 从服务器拉取修改..."
rsync -avz --exclude='.git' --exclude='node_modules' --exclude='__pycache__' --exclude='*.pyc' --exclude='.env' --exclude='.venv' --exclude='.vscode' --exclude='sessions' --exclude='config.json' ${SERVER}:${SERVER_PATH}/ ./
