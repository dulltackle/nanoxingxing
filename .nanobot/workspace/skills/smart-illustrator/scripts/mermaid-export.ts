 ...
 *   Uses system Chrome (faster, no Chromium download needed)
...
async function checkMermaidCli(): Promise<boolean> {
  return new Promise((resolve) => {
    const proc = spawn('npx', ['mmdc', '--version'], { stdio: 'pipe' });
...
async function exportMermaid(
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn('npx', ['mmdc', ...args], { stdio: 'inherit' });
...
async function main() {
  ...
  if (!hasMmdc) {
    ...
    console.error('Install locally: cd scripts && npm install');
    console.error('Or globally: npm install -g @mermaid-js/mermaid-cli');
    process.exit(1);
  }
